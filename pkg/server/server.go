...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger, enabledTools []string) *MCPServer {
	...
	if shouldAddTool(ToolConversationsAddMessage, enabledTools, "SLACK_MCP_ADD_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolConversationsAddMessage,
			...
			mcp.WithString("text",
				mcp.Description("Message body. With content_type 'text/markdown' (default) you MUST write standard Markdown (not Slack mrkdwn). Links: use Markdown [label](url) whenever you have a descriptive label; the server renders it as a native clickable Slack link (for example [DIL-689](https://ricloud.atlassian.net/browse/DIL-689)). Do not paste a bare URL when a label is available, and do not use Slack's <url|text> link syntax. Mentions MUST use Slack token syntax, never a bare @name: <@USER_ID> (user), <#CHANNEL_ID> (channel), <!subteam^GROUP_ID> (user group), <!here>/<!channel>/<!everyone> (broadcast). To mention a person, first call users_search to get their USER_ID; never invent an ID."),
			),
	...
	if shouldAddTool(ToolFilesSend, enabledTools, "SLACK_MCP_FILES_TOOL") {
		...
		s.AddTool(mcp.NewTool(ToolFilesSend,
			mcp.WithDescription("Upload and send a file to a Slack channel, DM, or thread. Supply the file via EXACTLY ONE of: file_path (server reads the file from disk; use this for real or binary files), content (inline UTF-8 text), or content_base64 (inline base64; only practical for tiny binaries). Disabled by default; enable with SLACK_MCP_FILES_TOOL. Uploads are limited to the same channels as SLACK_MCP_ADD_MESSAGE_TOOL. Maximum file size is 50MB."),
			...
			mcp.WithString("file_path",
				mcp.Description("Path the server reads the file from, e.g. /uploads/photo.jpg. PREFERRED for real or binary files: pass a path, never the bytes. The path MUST resolve inside the directory set by SLACK_MCP_FILES_PATH_ROOT (file_path is disabled when that is unset); host files must be staged into that mounted directory first. Mutually exclusive with content and content_base64."),
			),
			mcp.WithString("filename",
				mcp.Description("Name for the file (e.g., report.md, screenshot.png). Optional with file_path (defaults to the file's base name); required for content and content_base64."),
			),
			mcp.WithString("content",
				mcp.Description("Text content for the file. Use this for text-based files. Mutually exclusive with content_base64 and file_path."),
			),
			mcp.WithString("content_base64",
				mcp.Description("Base64-encoded content for small binary files. For real files use file_path instead; an LLM cannot reproduce large base64 reliably. Mutually exclusive with content and file_path."),
			),
			mcp.WithString("thread_ts",
			...
			mcp.WithString("initial_comment",
				mcp.Description("Optional message shown with the file. Write standard Markdown (not Slack mrkdwn); it is rendered as rich_text like conversations_add_message. Links: use Markdown [label](url) whenever you have a descriptive label; the server renders it as a native clickable Slack link (for example [DIL-689](https://ricloud.atlassian.net/browse/DIL-689)). Do not paste a bare URL when a label is available, and do not use Slack's <url|text> link syntax. Mentions MUST use Slack token syntax, never a bare @name: <@USER_ID> (user), <#CHANNEL_ID> (channel), <!subteam^GROUP_ID> (user group), <!here>/<!channel>/<!everyone> (broadcast). To mention a person, first call users_search to get their USER_ID; never invent an ID."),
			),

...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger, enabledTools []string) *MCPServer {
	...
	if shouldAddTool(ToolConversationsAddMessage, enabledTools, "SLACK_MCP_ADD_MESSAGE_TOOL") {
		s.AddTool(mcp.NewTool(ToolConversationsAddMessage,
			...
			mcp.WithString("text",
				mcp.Description("Message body. With content_type 'text/markdown' (default) you MUST write standard Markdown (not Slack mrkdwn). Mentions MUST use Slack token syntax, never a bare @name: <@USER_ID> (user), <#CHANNEL_ID> (channel), <!subteam^GROUP_ID> (user group), <!here>/<!channel>/<!everyone> (broadcast). To mention a person, first call users_search to get their USER_ID; never invent an ID."),
			),
	...
	if shouldAddTool(ToolFilesSend, enabledTools, "SLACK_MCP_FILES_TOOL") {
		...
		s.AddTool(mcp.NewTool(ToolFilesSend,
			...
			mcp.WithString("initial_comment",
				mcp.Description("Optional message shown with the file. Write standard Markdown (not Slack mrkdwn); it is rendered as rich_text like conversations_add_message. Mentions MUST use Slack token syntax, never a bare @name: <@USER_ID> (user), <#CHANNEL_ID> (channel), <!subteam^GROUP_ID> (user group), <!here>/<!channel>/<!everyone> (broadcast). To mention a person, first call users_search to get their USER_ID; never invent an ID."),
			),
